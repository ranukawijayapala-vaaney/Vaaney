...
- **Legal Documents:** Five public legal pages rendered via shared `client/src/components/LegalLayout.tsx` (back button, title, prose container) with content in `client/src/pages/legal/`: Privacy Policy (`/legal/privacy`, structured per Sri Lanka PDPA No. 9 of 2022), Buyer Product Purchase Terms (`/legal/buyer-products`), Buyer Service Booking Terms (`/legal/buyer-services`), Product Supplier Agreement (`/legal/seller-products`) and Service Supplier Agreement (`/legal/seller-services`). Routes registered in every Switch block in `App.tsx` so docs are accessible regardless of auth state. Footer with legal/guides links added to `GuestLayout`. Documents contain `[bracketed]` placeholders for company registration details, dates, and amounts that must be filled in before legal sign-off. Uses `@tailwindcss/typography` `prose` classes for readable rendering.
